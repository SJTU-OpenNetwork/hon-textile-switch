...
func (h *StreamService) Start() {
	h.peerHost.SetStreamHandler(streamServiceProtocol, h.handleNewStream)
}

func (h *StreamService) handleNewStream(s inet.Stream) {
	defer s.Reset()
	if h.handleNewMessage(s) {
...
func (h *StreamService) handleNewMessage(s inet.Stream) bool {
	...
	for {
		...
		if err := h.VerifyEnvelope(&req, mPeer); err != nil {
			continue
		}

		rpmes, err := h.handle(&req, mPeer)
		if err != nil {
...
func (h *StreamService) handleBlk(pid peer.ID, blk *pb.StreamBlock) error {
    // TODO: save blk as a file
//    if len(blk.Data) != 0 {
//        err := save(bytes.NewReader(blk.Data))
//        if err != nil {
//            return nil, err
//        }
//    }
